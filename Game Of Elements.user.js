...
var storageLimitsArray = [ { res : 'Elixier der Genesung',          y :   -1, g :   -1 }, { res : 'Elixier der Kraft',             y :   -1, g :   -1 },
                           ...
                           { res : 'Pflanzenkorn',                  y :   -1, g :   -1 }, { res : 'Portalstein',                   y :   -1, g :   -1 },
                           { res : 'roter Smaragd',                 y :   -1, g :   -1 }, { res : 'Silberlegierung',               y :   25, g :   50 },
                           ...
                           { res : 'Datteln',                       y :   -1, g :   -1 }, { res : 'Deben',                         y :   -1, g :   -1 },
...
var items = [ 'Angel des Donners', 'Angel des Schweigens', 'Bier', 'blaue Angel', 'blaues Geschenk', 'Blech', 'Blume', 'Blumenkranz', 'Blumenstrau\u00df', 'Donnerbeere',
...
// job variables ..
// stonecutter
var repOptimum = [90.1, 90.1, 93.4]; // repair optimum for each building level (1, 2, 3)

// alchemist
var healValue  = 600;
var alchemistSpellDefault = 0;
var alchemistPotionDefault = 0;

// engineer
var primaryEngineerItemMap = { 'Bronzelegierung' : '0', 'Eisenlegierung' : '1', 'Kupferlegierung' : '2', 'Zinnlegierung' : '3',
                               'Silberlegierung' : '4', 'Goldlegierung' : '5', 'Draht' : '6', 'Portalstein':'7' };
var defaultPrimaryEngineerItem = 'Eisenlegierung';

// farmer
var foodSelectionMap = { 'Datteln' : '0', 'Myrrhe' : '1', 'Sellerie' : '2', 'Honig' : '3' };
var defaultFoodSelection1 = 'Datteln';
var defaultFoodSelection2 = 'Datteln';
...
var retrieveAnimalDatesOfDeathVar = [];
// ---------------------------------------------------------------------------------------------------------
...
/*27*/  createConfigTableRow( configTable, 'Knapsack Optimierung bei "ausgeglichener Kampf" (Stadtprofil)',        createSwitch( 'doKnapsack', doKnapsackCalc ) );
        ...
        createConfigTableRow( jobTable, '', 'span' ); // engineer row
        ...
        getTableElement( jobTable, 3, 0 ).appendChild( createElementT( 'h2', 'Steinmetz:' ) );
        ...
        getTableElement( jobTable, 3, 0 ).appendChild( repairBLabel );

        // special engineer settings
        getTableElement( jobTable, 3, 0).appendChild( createElementT( 'h2', 'Ingenieur:' ) );
        var primaryEngineerItemLabel = createElementT( 'label', 'Gegenstand herstellen (Vorauswahl): ' );
        getTableElement( jobTable, 3, 0).appendChild( primaryEngineerItemLabel );
        var primaryEngineerItemSelection = createSelection( 'primaryEngineerItemCombo', primaryEngineerItemMap, String( defaultPrimaryEngineerItem ), updateConfig );
        //primaryEngineerItemSelection.style.marginLeft = '5px';
        getTableElement( jobTable, 3, 0 ).appendChild( primaryEngineerItemSelection );
...
{
    firstCssLink = document.getElementsByTagName( 'link' )[0];
...
// add / restire default link entries ..
function addDefaultLinkEntries()
{
...
if(checkHash(getUserName()))
    ...
    healValue = parseInt( document.getElementById( 'healValueInput' ).value );
    ...
    defaultPrimaryEngineerItem = document.getElementById( 'primaryEngineerItemCombo' ).value;
...
function saveConfig()
{
    ...
    GM_setValue( 'defaultPrimaryEngineerItem', defaultPrimaryEngineerItem );
...
function loadConfig()
{
    ...
    healValue = GM_getValue( 'healValue', healValue );

    repOptimum = JSON.parse( GM_getValue( 'repOptimum', JSON.stringify( repOptimum ) ) );

    defaultPrimaryEngineerItem = GM_getValue( 'defaultPrimaryEngineerItem', defaultPrimaryEngineerItem );

    defaultFoodSelection1 = GM_getValue( 'defaultFoodSelection1', defaultFoodSelection1 );
    defaultFoodSelection2 = GM_getValue( 'defaultFoodSelection2', defaultFoodSelection2 );

    // load jod selection defaults ..
    alchemistSpellDefault  = GM_getValue( 'alchemistSpellDefault', alchemistSpellDefault );
    alchemistPotionDefault = GM_getValue( 'alchemistPotionDefault', alchemistPotionDefault );
}
...
// move some page elements
function reorgPageElems()
{
    if( doReorgPageElems )
    {
...
function showAnimalWarning()
{
    if( displayAnimalWarning)
    {
        ...
        if( !document.URL.includes( 'show' ) && !document.URL.includes( 'ok=1' ) &&
		...
		for(var i = 0; i < msg.length; i++) {
			newWarning.appendChild( document.createTextNode( msg[i] ) );
...
// add a additional work link
function addAdditionalWorkLink( enabled, workValue )
{
   if( enabled )
   {
...
function forum()
{
    ...
    if( addBBCodes )
    {
        ...
        if( document.URL.includes( 'site=forum&answer=' ) || document.URL.includes( 'site=forum&edit=' ) || document.URL.includes( 'site=forum&katid=2&newthread=' ) ) // "Forum"
        {
        ...
        else if( document.URL.includes( 'site=gruppe&do=forum&newthread=' ) || document.URL.includes( 'site=gruppe&do=forum&edit=' ) || document.URL.includes( 'site=gruppe&do=forum&answer=' )) // "Stadtforum"
        {
            textArea.parentNode.insertBefore( newCodeBar, textArea.nextSibling );
...
{
    retrieveAnimalDatesOfDeathVar = []; // clear result object
    var frameA = createElementA( 'iframe', 'id', 'Tierverwaltung' );
    frameA.onload = function()
    {
        // get the data from the first page / table ..
...
function useRetrieveAnimalDatesOfDeathVar( datesOfDeathVar )
{
    ...
    {
        var dateA = new Date( a.date.split( '.' )[2], a.date.split( '.' )[1], a.date.split( '.' )[0] ); //parseInt( a.date.split( '.' )[2] + a.date.split( '.' )[1] + a.date.split( '.' )[0] );
        var dateB = new Date( b.date.split( '.' )[2], b.date.split( '.' )[1], b.date.split( '.' )[0] ); //parseInt( b.date.split( '.' )[2] + b.date.split( '.' )[1] + b.date.split( '.' )[0] );
        return dateA - dateB;
    } );
...
{
    if( addDateOfDeath && ( (document.URL.include == 'http://game-of-elements.de/index.php?site=tierverwaltung') ||
...
function getDateOfDeath( ageInformation )
{
    ...
    //return dateOfDeath.toLocaleDateString();
...
// "Tier füttern" page ..
function feedAnimal() {
	if(document.URL.includes( 'site=tierverwaltung' ) && document.URL.includes( '&food=' ))
	{
		// add input options for numbers
		var amountInput = getContent().getElementsByTagName( 'input' )[0];
		amountInput.setAttribute( 'type' , 'number' );
...
function transfer()
{
	...
	{
		var amountInput = getContent().getElementsByTagName( 'input' )[0]; // biete ich ..
		amountInput.setAttribute( 'type' , 'number' );
		amountInput.setAttribute( 'min' , '0' );
		amountInput.setAttribute( 'style' , 'height:16px; width:50px' );
		var userSelection = getContent().getElementsByTagName( 'select' )[0];
		userSelection.setAttribute( 'style' , 'height:19px; width:150px' );

		amountInput = getContent().getElementsByTagName( 'input' )[2]; // möchte ich ..
		amountInput.setAttribute( 'type' , 'number' );
		amountInput.setAttribute( 'min' , '0' );
		amountInput.setAttribute( 'style' , 'height:16px; width: 50px' );
		var userSelection = getContent().getElementsByTagName( 'select' )[1];
		userSelection.setAttribute( 'style' , 'height:19px; width:150px' );
	}
}
...
function marketplace()
{
	if(document.URL.includes( 'site=marktplatz&kaufen=' ))
	{
		...
		amountInput.setAttribute( 'type' , 'number' );
		amountInput.setAttribute( 'min' , '0' );
		amountInput.setAttribute( 'max' , amountInput.value );
		amountInput.setAttribute( 'step' , '1' );
...
function defaultSelectionsForAlchemist()
{
    // spell default
...
function preselectPrimaryEngineerItem()
{
   ...
   {
      document.form1.R1[primaryEngineerItemMap[String( defaultPrimaryEngineerItem )]].checked = true;
   }
}
...
// create a cobo box with the given options ..
...
// create a button ..
function createButton( id, text, fontSize, height, marginLeft, eventFunc )
{
...
// create a slider ..
function createSlider( id, min, max, step, width, value, eventFunc )
{
